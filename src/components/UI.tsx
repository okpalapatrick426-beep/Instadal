...
// ---------- Role switcher (respects allowedViews) ----------
...
// ---------- Auth page — real Supabase OTP ----------
export function AuthPage() {
  const { signUp, signIn, verifyOtp, loginAsGuest, loginAsStaff, partnerStaff, showToast } = useApp();
  const nav = useNavigate();
  ...
  const handleSendOtp = async () => {
    ...
    try {
      if (mode === "signup") {
        await signUp(name.trim(), phone.trim(), primaryRole);
      } else {
  ...
  const handleVerifyOtp = async () => {
    ...
    setLoading(true);
    try {
      const role = await verifyOtp(phone.trim(), otp.trim());
      nav("/" + role);
    } catch (e: unknown) {
    ...
    <div className="min-h-screen bg-[#FAFAFA]">
      ...
      <div className="mx-auto max-w-md px-4 -mt-2 pb-16">
        <div className="rounded-3xl bg-white border border-gray-100 shadow-xl overflow-hidden">
          ...
          <div className="p-5">
            {/* ── OTP verification step (shared between signup/signin) ── */}
            {otpSent ? (
              <>
                <div className="text-center mb-4">
                  <div className="text-3xl">📱</div>
                  <h2 className="text-lg font-extrabold mt-2">Check your phone</h2>
                  <p className="text-sm text-gray-500 mt-1">
                    We sent a 6-digit code to <span className="font-bold text-[#1A1A1A]">{phone}</span>
                  </p>
                </div>

                <label className="text-xs font-bold text-gray-500">Enter OTP</label>
                <input
                  value={otp}
                  onChange={(e) => setOtp(e.target.value)}
                  placeholder="123456"
                  maxLength={6}
                  className="mt-1 w-full rounded-xl border border-gray-200 bg-gray-50 p-3 text-sm outline-none focus:border-[#FF6B00] focus:bg-white text-center text-xl tracking-widest font-bold"
                />
                ...
                <button
                  ...
                  className="mt-3 w-full text-sm text-gray-500 hover:text-[#FF6B00]"
                >
                  ← Change number
                </button>
              </>
              ...
              <>
                ...
                >
                  {loading ? "Sending OTP..." : `Create ${roleOptions.find((o) => o.r === primaryRole)?.title} account`}
                </button>
              ...
              <>
                ...
                <div className="rounded-2xl border border-dashed border-orange-200 bg-orange-50/50 p-3">
                  ...
                  <button
                    onClick={staffSignIn}
                    className="mt-2 w-full rounded-full bg-[#1A1A1A] text-white py-2 text-xs font-bold"
                  >
                    Sign in as staff
                  </button>
                </div>
                ...
                <div className="pt-3 text-center">
                  <Link to="/admin/login" className="text-[11px] text-gray-400 hover:text-gray-600">
                    Admin access
                  </Link>
                </div>
              </>
...
export function LocationSelector({
  ...
  initialState?: string; initialCity?: string;
}) {
...
export function AddressCard({
  ...
  addr: SavedAddress; selected?: boolean;
  ...
  return (
    <button onClick={onSelect}
      ...
      {addr.mapsLink && (
        <a href={addr.mapsLink} target="_blank" rel="noopener noreferrer"
          onClick={(e) => e.stopPropagation()}
