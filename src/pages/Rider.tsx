...
export function RiderLogin() {
  ...
  const handleVerify = async () => {
    ...
    setLoading(true);
    try { await verifyOtp(phone.trim(), otp.trim()); nav("/rider"); }
    ...
    <div className="min-h-screen bg-gradient-to-br from-sky-50 via-white to-blue-50 grid place-items-center p-4">
      <div className="w-full max-w-sm">
        ...
        <div className="rounded-3xl bg-white p-6 shadow-xl border border-gray-100">
          <h2 className="text-xl font-extrabold">{otpSent ? "Enter OTP" : "Rider sign in"}</h2>
          <p className="text-sm text-gray-500">{otpSent ? `Code sent to ${phone}` : "Deliver & earn in Awka."}</p>
          {!otpSent ? (
            <>
