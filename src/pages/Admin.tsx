...
export function AdminLogin() {
  ...
  const nav = useNavigate();
  // Admin signs in via phone OTP like everyone else.
  // Their profile row has primary_role = 'admin' which gates the UI.
    ...
    <div className="min-h-screen bg-gradient-to-br from-violet-50 via-white to-purple-50 grid place-items-center p-4">
      <div className="w-full max-w-sm">
        ...
        <div className="rounded-3xl bg-white p-6 shadow-xl border border-gray-100">
          <h2 className="text-xl font-extrabold">{otpSent ? "Enter OTP" : "Admin sign in"}</h2>
          <p className="text-sm text-gray-500">{otpSent ? `Code sent to ${phone}` : "Manage the platform."}</p>
          {!otpSent ? (
            <>
